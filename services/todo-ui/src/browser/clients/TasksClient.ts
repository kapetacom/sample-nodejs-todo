...
export class TasksClient {
    ...
    getTasks(userId: string): Promise<Task[]> {
        return this.client.execute('GET', '/tasks/{userId}', [{ name: 'userId', value: userId, transport: 'PATH' }]);
    }
    ...
    removeTasks(userId: string): Promise<void> {
        return this.client.execute('DELETE', '/tasks/{userId}', [{ name: 'userId', value: userId, transport: 'PATH' }]);
    }
}
